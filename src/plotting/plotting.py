...
def get_best_coefficients_plotting(
    df,
    ...
    if task == "regression":
        ...
        sorted_df = df.sort_values(
            by=[metric, "Dataset", "Layer", "Inputs", "Match-Type"],
            ascending=[metric_sorting, True, True, True, False],
        )
    else:
        sorted_df = df.sort_values(
            by=[metric, "Dataset", "Layer", "Inputs", "Match-Type"],
    ...
    df_best_coefficients = sorted_df.groupby(
        ["Dataset", "Layer", "Inputs", "Match-Type"]
    )[
        [
            "Coefficients",
            "Dataset",
    ...
    if select:
        ...
        return df_best_coefficients[
            (df_best_coefficients["Dataset"].str.contains(task_name))
            & (df_best_coefficients["Inputs"] == "Activations")
        ].Coefficients.values
...
def rename_group(s):
    return (
        s.replace("gemma-2-2b", "Gemma-2-2B")
        .replace("it", "IT")
        ...
        .replace("YES NO QUESTION", "YES/NO")
...
def plot_probe_performance_by_model_family(
    ...
    for task, metric in task_types.items():
        ...
        for ix, (filename, model_name) in enumerate(filename_pairs):
            ...
            df = postprocess_df_probes(
                pd.read_pickle(f"../runs/probes/{filename}.pkl"),
                filter_error_type=error_type,
                filter_probe_match_type="Last",
            )
            df_exact = postprocess_df_probes(
                pd.read_pickle(f"../runs/probes/{filename}.pkl"),
                filter_error_type=error_type,
                filter_probe_match_type="Exact",
            )
            ...
            if not PLOT_SAEs:
                df_best_coefficients = df_best_coefficients.loc[
                    df_best_coefficients["Inputs"] == "Activations"
                ]
            ...
            if (ix + 1) % 2 == 0:
                ...
                df_family["Token position"] = df_family["Match-Type"].replace(
                    ["Last", "Exact"], ["Last (prompt)", "Exact (generation)"]
                )
                if "Inputs" in df_family:
                    col = "Representation"
                    df_family["Representation"] = df_family["Inputs"].replace(
                        ["Activations", "Encodings"], ["Activations", "Sparse rep."]
                    )
                ...
                sns.lineplot(
                    ...
                    palette=(
                        ["black", "black"]
                        if not PLOT_SAEs
                        else ["#1f77b4", "#2ca02c", "#1f77b4", "#2ca02c"]
                    ),
                )
        ...
        for y_axis in [f"{metric}"]:
            for filename, model_name in filename_pairs:
                ...
                fig, axes = plt.subplots(
                    1, len(task_names), figsize=(2.5 * len(task_names), 2), sharey=False
                )
                if len(task_names) == 1:
                    axes = [axes]
                ...
                for index, (ax, dataset) in enumerate(zip(axes, task_names)):
                    ...
                    df_best_coefficients["Token position"] = df_best_coefficients[
                        "Match-Type"
                    ].replace(
                        ["Last", "Exact"], ["Last (prompt)", "Exact (generation)"]
                    )
                    if "Inputs" in df_best_coefficients:
                        df_best_coefficients["Features"] = df_best_coefficients[
                            "Inputs"
                        ].replace(
                            ["Activations", "Encodings"], ["Activations", "Sparse rep."]
                        )
                ...
                save_path_png = (
                    f"../runs/plots/probe-{model_name}-{metric}_{nr_rows}.png"
                )
                save_path_svg = (
                    f"../runs/plots/probe-{model_name}-{metric}_{nr_rows}.svg"
                )
                plt.savefig(save_path_png)
                plt.savefig(save_path_svg)
                plt.show()
